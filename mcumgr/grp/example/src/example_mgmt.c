    ...
    /* The below should be updated with the real name of the file */
    #include <<grp_name>_mgmt_callbacks.h>
    #endif
    #endif
    ...
    static int example_mgmt_test(struct smp_streamer *ctxt)
    {
         ...
         * empty
         */
        if (!ok || string_value.len == 0 || !zcbor_map_decode_bulk_key_found(example_test_decode,
            ARRAY_SIZE(example_test_decode), "uint_key")) {
        ...
        ok = zcbor_tstr_put_lit(ctx->zse, "return_int") &&
             zcbor_int32_put(ctx->zse, 4691);
    ...
    static int example_mgmt_other(struct smp_streamer *ctxt)
    {
        ...
        ok = zcbor_tstr_put_lit(ctx->zse, "return_string")     &&
             zcbor_tstr_put_lit(ctx->zse, "some dummy data!");
     ...
     * only for general SMP/MCUmgr errors. The success/OK error code is not used in this
     * translation function as it is automatically handled by the base SMP code.
     */
